...
def parse(file, section, tag=None) :
    ...
    if not file : file = os.getenv("DES_SERVICES")
    if not file : file = os.path.join(os.getenv("HOME"),".desservices.ini")
    if not section and tag : section = os.getenv("DES_%s_SECTION" % tag.upper())
...
def check(d, tag=None):
    ...
    if permission_faults :  raise ServiceaccessException(
        "faulty permissions : %s " % (permission_faults))
    if tag and tag.lower() == "db" : _check_db(d)
...
def _process_db(d):
    ...
    if d["type"] == "oracle"   : d.setdefault("port", "1521")
    if d["type"] == "postgres" : d.setdefault("port", "5432")
    return d
...
def _check_db(d):
    ...
    for key in d.keys():
        if key not in expected_db_keys:
            extra.append(key)
    check (d)
    if missing or extra : raise ServiceaccessException("faulty keys : %s " % (missing, extra))
    return 
